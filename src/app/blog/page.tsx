...
export default async function BlogIndexPage(props: { searchParams: Promise<{ [key: string]: string | string[] | undefined }> }) {
  ...
  return (
    <div className="min-h-screen bg-matte-black pb-24 overflow-hidden">
      {/* Banner */}
      <div
        className="relative pt-32 pb-16 px-6 lg:px-8 mb-16 bg-cover bg-no-repeat bg-center bg-gray-900"
        style={{ backgroundImage: 'url("/blog.png")' }}
      >
        <div className="absolute inset-0 bg-black/60 pointer-events-none" />
        <div className="relative max-w-7xl mx-auto" style={{ zIndex: 100 }}>
          <h1
            className="text-4xl md:text-5xl font-serif font-black tracking-wide text-center lg:text-left drop-shadow-xl"
            style={{ color: '#ffffff' }}
          >
            Blog
          </h1>
                ...
                {posts.map((post: any) => (
                  ...
                  >
                    ...
                    <div className="p-5 flex flex-col flex-1">
                      <div className="flex items-center justify-between text-xs text-gray-500 mb-2 uppercase tracking-widest font-semibold">
                        <span className="opacity-0 hidden">Author</span>
                        <span>{new Date(post.createdAt).toISOString().split('T')[0]}</span>
                      </div>
                      <h2 className="text-lg font-serif text-white font-bold mb-2 line-clamp-2 group-hover:text-[#D0A511] transition-colors">
                        {post.title}
                      </h2>
